...
{
	@Query(value="Select * from tbl_billing_logs where status='Failed' and failedStatus=:status and date(DATETIME)=date(curdate())",nativeQuery=true)
	public List<TblBillingLogs> getForFailed(@Param("status") String status);
